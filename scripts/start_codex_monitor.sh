...
ensure_devtools() {
  ...
  if [[ "${observed_pid}" == "${DEFERRED_RELAUNCH_PID}" ]] && (( SECONDS < RETRY_AFTER )); then
    return 1
  fi

  if ! "${SCRIPT_DIR}/reopen_codex_with_debug.sh" "${PORT}"; then
    # A busy app can defer a normal quit. Retry later rather than disabling the
    # HUD for this entire app session; never force-quit or retry in a tight loop.
    DEFERRED_RELAUNCH_PID="${observed_pid}"
